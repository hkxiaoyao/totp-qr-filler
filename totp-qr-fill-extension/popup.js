...
const OTP_STORAGE_KEY = "activeOtp";
let activeOtp = null;
...
async function restoreStoredOtp() {
  const stored = await chrome.storage.session.get(OTP_STORAGE_KEY);
  const otp = stored[OTP_STORAGE_KEY];
...
async function saveActiveOtp(otp) {
  await chrome.storage.session.set({ [OTP_STORAGE_KEY]: otp });
}
...
async function fillCodeInPage(tabId, code) {
  const runFill = (value) => {
    if (typeof globalThis.ephemeral2faFillCode !== "function") {
      return { ok: false, reason: "填充脚本未加载。" };
    }

    return globalThis.ephemeral2faFillCode(value);
  };
...
async function detectQrFromDataUrl(dataUrl) {
  ...
  if (typeof jsQR !== "function") {
    throw new Error("当前 Chrome 不支持 BarcodeDetector，且 jsQR 备用解码器未加载。");
  }
